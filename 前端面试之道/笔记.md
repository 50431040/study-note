...
JS 引擎线程和渲染线程是互斥的，如果 JS 执行的时候 UI 线程还在工作，可能导致不能安全的渲染 UI。可以节省内存，节约上下文切换事件，没有锁的微问题。
...
满足以下条件时，会触发简单请求：使用`GET`、`HEAD`、`POST`方法之一、`Content-Type` 的值只能为 `text/plain`，`multipart/form-data`，`application/x-www-forn-urlencoded` 之一。其他的都为复杂请求。
...
Disk Cache 覆盖面基本是最大的。它会根据 HTTP Hearder 中的字段判断哪些资源需要缓存，哪些资源可以不请求直接使用，哪些资源已经过期需要重新请求。并且即时在跨站点的情况下，相同地址的资源一旦被硬盘缓存下来，就不会再次去请求。
